        ...
        if len(soup_data) > 0:
            ...
            for detail in game_details:
                ...
                if "Developer" in detail:
                    splitted_links = detail.split('</div>')[0].split('</b>')[1].split(",  <a")
                    data_array = []
                    for link in splitted_links:
                        data_array.append(link.split(
                            "\">")[1].replace('</a>', ''))
                ...
                if "Publisher" in detail:
                    splitted_links = detail.split('</div>')[1].split('</b>')[1].split(", <a")
                    data_array = []
                    for link in splitted_links:
                        data_array.append(link.split(
                            "\">")[1].replace('</a>', ''))
                ...
                if "Release" in detail:
                    if "Franchise" in detail:
                        release_date = detail.split('</div>')[3].split('</b>')[1]
                    else:
                        release_date = detail.split('</div>')[2].split('</b>')[1]
